...
import { adminEffectTypes, type EffectFieldDefinitionPayload, type EffectTypePayload } from '@/app/lib/adminData'
import type { EffectFieldTypeValue, EffectType, SelectOption } from '@/app/lib/gameData'
...
const FIELD_TYPE_LABELS: Record<EffectFieldTypeValue, string> = {
  ...
  entity: 'Entidade', effect: 'Efeito', target: 'Alvo', direction: 'Direção',
}

const OPTIONS_FIELD_TYPES = new Set<EffectFieldTypeValue>(['select', 'multi_select'])

const EMPTY_FIELD: EffectFieldDefinitionPayload = {
  name: '', label: '', description: '', field_type: 'text',
  is_required: false, default_value: '', min_value: null, max_value: null,
  options: null, sort_order: 0,
}

const EMPTY: EffectTypePayload = {
  name: '', slug: '', description: '', field_definitions: [],
}

export default function AdminTiposDeEfeitoPage() {
  const { token } = useAuth()
  const [effectTypes, setEffectTypes] = useState<EffectType[]>([])
  const [loading, setLoading] = useState(true)
  const [editingId, setEditingId] = useState<number | null>(null)
  const [form, setForm] = useState<EffectTypePayload>(EMPTY)
  const [error, setError] = useState<string | null>(null)
  const [saving, setSaving] = useState(false)

  const load = () => adminEffectTypes.list(token).then(setEffectTypes).finally(() => setLoading(false))
  useEffect(() => { load() }, [token])

  function startCreate() { setEditingId(0); setForm(EMPTY); setError(null) }
  function startEdit(effectType: EffectType) {
    setEditingId(effectType.id)
    setForm({
      name: effectType.name,
      slug: effectType.slug,
      description: effectType.description,
      field_definitions: effectType.field_definitions.map(f => ({ ...f })),
    })
    setError(null)
  }
  ...
  function addField() {
    setForm({ ...form, field_definitions: [...form.field_definitions, { ...EMPTY_FIELD, sort_order: form.field_definitions.length }] })
  }
  function updateField(i: number, patch: Partial<EffectFieldDefinitionPayload>) {
  ...
  async function save() {
    ...
    setError(null)
    try {
      if (editingId) await adminEffectTypes.update(token, editingId, form)
      else await adminEffectTypes.create(token, form)
      await load()
      cancel()
    } catch (err) {
  ...
  async function remove(id: number) {
    try {
      await adminEffectTypes.remove(token, id)
      await load()
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6" style={{ maxWidth: 1100 }}>
      <div className="flex items-center justify-between">
        <h1 style={{ fontFamily: 'var(--font-cinzel-decorative)', fontSize: '1.4rem', color: 'var(--text)' }}>Tipos de Efeito</h1>
        {editingId === null && <button onClick={startCreate} className="btn-hero" style={{ fontSize: '0.7rem', padding: '0.55rem 1.2rem' }}>+ Novo Tipo de Efeito</button>}
      </div>

      <p style={{ fontFamily: 'var(--font-im-fell)', fontStyle: 'italic', color: 'rgba(var(--text-rgb),0.55)', maxWidth: 700 }}>
        Cada tipo de efeito descreve os campos necessários para configurá-lo (ex: Dano precisa de uma quantidade, Aura precisa de um raio). O CMS usa essas definições para montar o formulário de cada Efeito automaticamente.
      </p>
      ...
      {editingId !== null && (
        <div className="card" style={{ padding: '1.25rem', borderRadius: 10 }}>
          <h2 style={{ fontFamily: 'var(--font-cinzel)', fontSize: '0.9rem', color: 'var(--gold)', marginBottom: '1rem' }}>
            {editingId ? 'Editar Tipo de Efeito' : 'Novo Tipo de Efeito'}
          </h2>
            ...
            {form.field_definitions.map((field, i) => (
              <div key={i} className="card" style={{ padding: '0.85rem', borderRadius: 8, background: 'var(--bg-secondary)' }}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  ...
                  <Field label="Ordem">
                    <Input type="number" value={field.sort_order} onChange={e => updateField(i, { sort_order: Number(e.target.value) })} />
                  </Field>
      ...
      {loading ? <p style={{ color: 'var(--text-muted)' }}>Carregando...</p> : (
        <AdminTable headers={['Nome', 'Slug', 'Campos', '']}>
          {effectTypes.map(effectType => (
            <Tr key={effectType.id}>
              <Td>{effectType.name}</Td>
              <Td>{effectType.slug}</Td>
              <Td>{effectType.field_definitions.length}</Td>
              <Td>
                <div className="flex items-center gap-2">
                  <button onClick={() => startEdit(effectType)} className="ddb-badge ddb-badge-gold" style={{ border: 'none', cursor: 'pointer' }}>Editar</button>
                  <ConfirmButton onConfirm={() => remove(effectType.id)} className="badge badge--error" style={{ border: 'none', cursor: 'pointer' }}>Excluir</ConfirmButton>
                </div>
              </Td>
            </Tr>
          ))}
