...
const SoulSvg = () => (
  <svg width="40" height="40" viewBox="0 0 32 32" fill="none" style={{ margin: '0 auto .5rem' }} aria-hidden>
    <circle cx="16" cy="16" r="13" fill="rgba(var(--gold-rgb),.08)" stroke="rgba(var(--gold-rgb),.35)" strokeWidth="1" />
    <ellipse cx="16" cy="16" rx="4" ry="6" fill="none" stroke="rgba(var(--gold-rgb),.5)" strokeWidth="1.2" />
    <circle cx="16" cy="16" r="1.8" fill="rgba(var(--gold-rgb),.7)" />
  </svg>
)

export default function LegadoSection() {
  return (
    <section id="legado" aria-labelledby="legado-title" style={{ background: 'var(--bg)', padding: '5rem 0' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
          ...
          >
            ...
            <div className="flex flex-col gap-2" style={{ width: '75%' }} aria-hidden>
              <div style={{ height: 10, width: '70%', borderRadius: 4, background: 'rgba(var(--gold-rgb),.08)', border: '1px solid rgba(var(--gold-rgb),.15)' }} />
              <div style={{ height: 10, width: '55%', borderRadius: 4, background: 'rgba(var(--gold-rgb),.1)', border: '1px solid rgba(var(--gold-rgb),.12)' }} />
              <div style={{ height: 10, width: '85%', borderRadius: 4, background: 'rgba(var(--gold-rgb),.1)', border: '1px solid rgba(var(--gold-rgb),.12)' }} />
              <div className="grid grid-cols-2 gap-1.5 mt-1" style={{ width: '100%' }}>
                {[0, 1, 2, 3].map((k) => (
                  <div key={k} style={{ height: 34, borderRadius: 6, background: 'rgba(var(--gold-rgb),.06)', border: '1px solid rgba(var(--gold-rgb),.1)' }} />
                ))}
              </div>
              <div style={{ height: 10, width: '60%', borderRadius: 4, marginTop: 4, background: 'rgba(var(--gold-rgb),.1)', border: '1px solid rgba(var(--gold-rgb),.12)' }} />
              <div style={{ height: 10, width: '55%', borderRadius: 4, background: 'rgba(var(--gold-rgb),.1)', border: '1px solid rgba(var(--gold-rgb),.12)' }} />
              <div style={{ height: 10, width: '75%', borderRadius: 4, background: 'rgba(var(--gold-rgb),.1)', border: '1px solid rgba(var(--gold-rgb),.12)' }} />
            </div>
          </div>
